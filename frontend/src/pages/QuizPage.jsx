  ...
  return (
    <div className="space-y-6">
      <section className="glass-card p-6">
        ...
        <h1 className="mt-2 font-display text-3xl font-bold text-ink">Practice at the right difficulty</h1>
        <div className="mt-6 grid gap-4 md:grid-cols-[1fr_1fr_auto]">
          ...
          <button type="button" onClick={generateQuiz} className="btn-primary">
            ...
            <div key={item.id} className="glass-card p-6">
              <div className="flex items-center justify-between gap-4">
                <div>
                  ...
                  <h2 className="mt-2 text-lg font-semibold text-ink">{item.question}</h2>
                </div>
                <span className="rounded-full bg-amber-50 px-3 py-1 text-xs font-semibold text-amber-700">{item.difficulty}</span>
              </div>
              ...
              {result ? (
                <div className="mt-4 rounded-2xl bg-slate-50 p-4 text-sm text-slate-600">{item.explanation}</div>
              ) : null}
            </div>
          ))}

          <button type="button" onClick={submitQuiz} className="btn-primary w-full">
            Submit Quiz
          </button>
        </section>
      ) : null}
      ...
      {result ? (
        <section className="glass-card p-6">
          ...
          <p className="mt-3 text-slate-600">Accuracy: {Math.round(result.accuracy)}%. Updated topics: {result.updated_topics.join(", ")}.</p>
        </section>
      ) : null}
    </div>
  );
